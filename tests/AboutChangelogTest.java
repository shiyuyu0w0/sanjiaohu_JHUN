...
public final class AboutChangelogTest {
    ...
    public static void main(String[] args) {
        ...
        String[] versions={"v1.1.6","v1.1.5","v1.1.4","v1.1.3","v1.1.2","v1.1.1","v1.1.0"};
        ...
        if(updates.length>=7){
            check(updates[0][1].contains("Gitee")&&updates[0][1].contains("登录失败")&&updates[0][1].contains("实验报告"),"v1.1.6 records download, login and report changes");
            check(updates[1][1].contains("北区18舍")&&updates[1][1].contains("电表"),"v1.1.5 records dorm meter fixes");
            check(updates[2][1].contains("校准")&&updates[2][1].contains("实验报告"),"v1.1.4 records schedule and upload fixes");
            check(updates[3][1].contains("刷新")&&updates[3][1].contains("考试"),"v1.1.3 records visual improvements");
            check(updates[4][1].contains("检查新版本")&&updates[4][1].contains("安装包"),"v1.1.2 records the updater");
            check(updates[5][1].contains("课表")&&updates[5][1].contains("跨校区"),"v1.1.1 records fixes");
            check(updates[6][1].contains("考试查询")&&updates[6][1].contains("电量"),"v1.1.0 records new functions");
            check(!updates[3][1].contains("检查新版本"),"v1.1.2 updater is not listed as new in v1.1.3");
        }
