...
final class ThemeColorSheet {
    ...
    ThemeColorSheet(MainActivity activity){
        ...
        TextView caption=a.label("颜色代码",12,a.MUTED,false);caption.setPadding(0,a.dp(6),0,a.dp(7));body.addView(caption);hex=new EditText(a);hex.setSingleLine(true);hex.setTextSize(16);hex.setTextColor(a.INK);hex.setHintTextColor(a.MUTED);hex.setHint("#2ECBFF");hex.setPadding(a.dp(14),0,a.dp(14),0);hex.setBackground(a.shape(a.palette.controlSurface,13));hex.setInputType(android.text.InputType.TYPE_CLASS_TEXT|android.text.InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS);hex.setFilters(new InputFilter[]{new InputFilter.LengthFilter(7)});body.addView(hex,new LinearLayout.LayoutParams(-1,a.dp(46)));error=a.label("",12,a.palette.error,false);error.setPadding(0,a.dp(8),0,0);body.addView(error);
    ...
    void update(){
        updating=true;ThemePalette theme=AppTheme.from(a,color);int[] surfaces={color,theme.deepAccent,theme.entrySurface};
