...
final class WeekSwipeLayout extends FrameLayout {
    ...
    final WeekSwipeGesture gesture;final Listener listener;VelocityTracker velocity;boolean transitioning,swipeEnabled=true;
    ...
    void setSwipeEnabled(boolean enabled){swipeEnabled=enabled;if(!enabled){gesture.cancel();release();}}
    @Override public boolean dispatchTouchEvent(MotionEvent e){
        if(!swipeEnabled)return super.dispatchTouchEvent(e);
    ...
    @Override public boolean onInterceptTouchEvent(MotionEvent e){return swipeEnabled&&(transitioning||e.getActionMasked()==MotionEvent.ACTION_MOVE&&gesture.move(e.getX(),e.getY()));}
