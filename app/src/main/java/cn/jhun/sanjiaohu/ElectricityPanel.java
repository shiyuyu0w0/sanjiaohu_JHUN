...
final class ElectricityPanel {
    ...
    final LinearLayout content,cards;final TextView buildingButton,floorButton,roomButton,hint;
    ...
    final ElectricityApi.Meter[] meters=new ElectricityApi.Meter[2];
    final ElectricityApi.Reading[] readings=new ElectricityApi.Reading[2];
    ...
    ElectricityPanel(IdentityActivity a){
        ...
        LinearLayout row=a.row(),left=a.column(),right=a.column();left.addView(a.text("楼层",12,t.muted,false));right.addView(a.text("房间",12,t.muted,false));a.gap(left,8);a.gap(right,8);
        floorButton=a.action("选择楼层  ›",false,()->chooseFloor());roomButton=a.action("选择房间  ›",false,()->chooseRoom());left.addView(floorButton,new LinearLayout.LayoutParams(-1,a.dp(52)));right.addView(roomButton,new LinearLayout.LayoutParams(-1,a.dp(52)));
        ...
        cards=a.column();content.addView(cards);a.gap(content,20);content.addView(a.text("空调与照明分别计量、分别缴费。\n从支付宝返回后可刷新电量，到账以学校系统为准。",12,t.muted,false));a.gap(content,14);
    ...
    void show(){a.setWebVisible(false);a.form.setVisibility(View.GONE);a.errorPanel.setVisibility(View.GONE);view.setVisibility(View.VISIBLE);a.subtitle.setText(connected?"宿舍空调 / 照明 · 剩余电量":"正在连接电费服务…");}
    ...
    void catalog(){clearReadings();request("正在获取宿舍楼…",ElectricityApi::buildings,data->{buildings=data;if(!buildings.containsKey(building)){building="";floor="";room="";}buttons();if(!building.isEmpty())loadFloors();else hint.setText("请选择宿舍楼、楼层和房间");});}
    void loadFloors(){List<ElectricityApi.Branch> selected=new ArrayList<>(buildings.get(building));floors=null;rooms=null;clearReadings();request("正在获取楼层…",api->api.floors(selected),data->{floors=data;if(floor.isEmpty()||!floors.containsKey(floor)){floor="";room="";}if(!floor.isEmpty())loadRooms();else hint.setText("请选择楼层");});}
    ...
    void query(){
        ...
        request("正在查询 "+building+" "+room+"…",api->{ElectricityApi.Reading[] r=new ElectricityApi.Reading[2];for(int i=0;i<2;i++)if(selected[i]!=null)r[i]=api.reading(selected[i]);return r;},data->{
            System.arraycopy(data,0,readings,0,2);boolean expired=false,failed=false;for(ElectricityApi.Reading r:data)if(r!=null){expired|=r.expired;failed|=!r.ok;}
            hint.setText(expired?"登录已过期，请重新登录":failed?"部分电表查询失败，可点击右上角刷新":building+" · "+floor+" 层 · "+room+" 室");hint.setTextColor(failed?t.error:t.muted);renderCards();
        });
    }
    void refresh(){if(!connected||busy)return;if(buildings==null)catalog();else if(!building.isEmpty()&&floors==null)loadFloors();else if(!floor.isEmpty()&&rooms==null)loadRooms();else if(!room.isEmpty())query();else hint.setText("请选择宿舍楼、楼层和房间");}
    void buttons(){
        buildingButton.setText(building.isEmpty()?"选择宿舍楼  ›":building+"  ›");floorButton.setText(floor.isEmpty()?"选择楼层  ›":floor+" 层  ›");roomButton.setText(room.isEmpty()?"选择房间  ›":room+" 室  ›");
    ...
    void chooseFloor(){if(floors!=null)choices("选择楼层",new ArrayList<>(floors.keySet()),value->{floor=value;room="";save();loadRooms();});}
    ...
    void renderCards(){
        cards.removeAllViews();for(int i=0;i<2;i++){
            ...
            View stripe=new View(a);stripe.setBackground(a.shape(i==0?0xff2f6fed:0xfff08a24,3));head.addView(stripe,new LinearLayout.LayoutParams(a.dp(4),a.dp(22)));
            TextView title=a.text(i==0?"空调电费":"照明电费",18,t.text,true);title.setPadding(a.dp(10),0,a.dp(8),0);head.addView(title,new LinearLayout.LayoutParams(0,-2,1));
            TextView pay=a.action("缴费",true,()->pay(index));pay.setContentDescription((i==0?"空调":"照明")+"缴费");enable(pay,reading!=null&&reading.ok&&reading.canBuy&&meter!=null&&!checkout);head.addView(pay,new LinearLayout.LayoutParams(a.dp(70),a.dp(44)));card.addView(head);a.gap(card,16);
    ...
    void pay(int index){
        ...
        a.web.evaluateJavascript(adapter+"(\"inspect\",null)",raw->{
            ...
            LinearLayout box=sheet(index==0?"空调缴费":"照明缴费");box.addView(a.text(building+" · "+room+" 室",16,t.text,true));a.gap(box,6);box.addView(a.text("电表 "+selected.room.id,12,t.muted,false));a.gap(box,20);
