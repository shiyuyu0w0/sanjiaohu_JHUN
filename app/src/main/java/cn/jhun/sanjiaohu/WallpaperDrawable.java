...
public final class WallpaperDrawable extends Drawable {
    ...
    private final int gutter,header,stripColor,scrimColor;
    ...
    public WallpaperDrawable(Bitmap bitmap,int gutter,int header,int stripColor,int scrimColor){this.bitmap=bitmap;this.gutter=gutter;this.header=header;this.stripColor=stripColor;this.scrimColor=scrimColor;}
    ...
    @Override public void draw(Canvas canvas){Rect b=getBounds();float scale=Math.max(b.width()/(float)bitmap.getWidth(),b.height()/(float)bitmap.getHeight());float w=bitmap.getWidth()*scale,h=bitmap.getHeight()*scale;canvas.save();canvas.clipRect(b);canvas.drawBitmap(bitmap,null,new RectF(b.centerX()-w/2,b.centerY()-h/2,b.centerX()+w/2,b.centerY()+h/2),paint);if(Color.alpha(scrimColor)>0){Paint shade=new Paint();shade.setColor(scrimColor);canvas.drawRect(b.left,b.top,b.right,b.bottom,shade);}Paint strip=new Paint();strip.setColor(stripColor);canvas.drawRect(b.left,b.top,b.right,b.top+Math.min(header,b.height()/5),strip);canvas.drawRect(b.left,b.top,b.left+Math.min(gutter,b.width()/5),b.bottom,strip);canvas.restore();}
